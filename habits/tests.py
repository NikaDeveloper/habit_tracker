...
class HabitTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create(email='test@example.com', is_staff=False, is_superuser=False)
    ...
    def test_validator_time_limit(self):
        ...
        self.assertIn('Время выполнения должно быть не больше 120 секунд', str(response.data))
    ...
    def test_validator_reward_and_related(self):
        ...
        response = self.client.post(self.list_url, data)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn('Нельзя одновременно выбрать связанную привычку и вознаграждение', str(response.data))
    ...
    def test_validator_related_is_pleasant(self):
        ...
        useful_habit = Habit.objects.create(
            user=self.user, place='Работа', time='09:00:00',
            action='Работа', is_pleasant=False, time_to_complete=60
        )
        data = self.habit_data.copy()
        del data['reward']  # Убираем награду, чтобы не сработал предыдущий валидатор
        data['related_habit'] = useful_habit.id  # Пытаемся привязать полезную, а не приятную
    ...
    def test_public_habits(self):
        ...
        Habit.objects.create(
            user=self.user, place='Улица', time='15:00:00',
            action='Бег', is_pleasant=False, time_to_complete=60, is_public=True
        )
